...
class Extractor(object):
    ...
    """
    def __init__(self, id, revid, title, lines, mark='none'):
        """
        :param id: id of page.
    ...
    def write_output(self, out, text):
        ...
        if options.write_json:
            ...
            if options.print_revision:
                header = '<doc id="%s" revid="%s" url="%s" title="%s" mark="%s">\n' % (self.id, self.revid, url, self.title, self.mark)
            else:
                header = '<doc id="%s" url="%s" title="%s" mark="%s">\n' % (self.id, url, self.title, self.mark)
            footer = "\n</doc>\n"
            if out == sys.stdout:   # option -a or -o -
    ...
    def wiki2text(self, text):
        ...
        # text = onlyKeepInternalLinks(text)
...
def replaceInternalLinksWithTracker(text):
    cur = 0
    res = ''
    mention_tracker = []
    for s, e in findBalanced(text):
        ...
        if pipe < 0:
            ...
            for s1, e1 in findBalanced(inner):
                ...
                curp = e1
            label = inner[pipe + 1:].strip()
        mention = makeInternalLink(title, label)
        res += text[cur:s]
        # mention start, end, mention text, entity text
        track = (len(res), len(res)+len(mention), title, mention)
        if mention and len(mention) > 0:
            mention_tracker.append(track)
        res += mention + trail
        cur = end
    return res + text[cur:], mention_tracker

def onlyKeepInternalLinks(text):
    cur = 0
    res = ''
    links = []
    for s, e in findBalanced(text):
...
def load_templates(file, output_file=None):
    ...
    for page_count, page_data in enumerate(pages_from(file)):
        id, revid, title, ns,catSet, page, _ = page_data
    ...
    for line in input:
        ...
        if tag == 'page':
            ...
            if id != last_id:
                if redirect:
                    mark = 'redirect'
                elif isDisambigPage(page):
                    mark = 'disambig'
                else:
                    mark = 'article'
                yield (id, revid, title, ns,catSet, page, mark)
                last_id = id
                ns = '0'
            # if redirect:
            #     print(id, revid, title, ns, catSet, page)
            id = None
            revid = None
            title = None
            page = []
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    for page_data in pages_from(input):
        id, revid, title, ns, catSet, page, mark = page_data
        if keepPage(ns, catSet, page):
            ...
            job = (id, revid, title, page, mark, page_num)
...
def extract_process(opts, i, jobs_queue, output_queue):
    ...
    while True:
        job = jobs_queue.get()  # job is (id, revid, title, page, mark, page_num)
        if job:
            id, revid, title, page, mark, page_num = job
            try:
                e = Extractor(*job[:5]) # (id, revid, title, page, mark)
                page = None              # free memory
                e.extract(out)
                text = out.getvalue()
            except:
